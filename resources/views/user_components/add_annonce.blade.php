...
<section class="max-w-4xl  p-6 mx-auto bg-indigo-600 rounded-md shadow-md dark:bg-gray-800 mt-20">
    ...
    <form class='mt-10' action="{{ route('publisher_components.annonce_store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
            ...
            <div>
                ...
                <input  type="text" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring" name="titre"  required>
            </div>
            <div >
            ...
            <!-- <div>
    <label class="text-white dark:text-gray-200" for="user_id">User</label>
    <select disabled class="appearance-none block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring" name='user_id'>
        ...
        @foreach ($users as $us)
            <option value="{{ $us->id }}" {{ (Auth::user()->id == $us->id) ? 'selected' : '' }} disabled>
                {{ $us->name }}
            </option>
        @endforeach
    </select>
        ...
        <div class="flex justify-end mt-8">
            <button class="px-6 py-2 leading-5 text-white transition-colors duration-200 transform bg-green-500 rounded-md hover:bg-green-900 focus:outline-none focus:bg-gray-600" type="submit">Enregistrer</button>
        </div>
    </form>
</section>
